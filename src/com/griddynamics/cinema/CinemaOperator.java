...
public class CinemaOperator {
    ...
    private int getPurchasedTicketsAmount() {
        return (int)getCinema().getHall().getRows().stream()
    ...
    private int getCurrentIncome() {
        final int purchasedFullPriceTicketsAmount = (int)getCinema().getHall().getRows().stream()
                .limit(getCinema().getHall().getLastFullPriceSeatsRowPosition())
                .flatMap(row -> row.getSeats().stream())
                .filter(Seat::isReserved)
                .count();
        final int purchasedDiscountPriceTicketsAmount = (int)getCinema().getHall().getRows().stream()
    ...
    private int getTotalIncome() {
        final int fullPriceSeatsRowsAmount = getCinema().getHall().getLastFullPriceSeatsRowPosition();
        final int discountPriceSeatsRowsAmount = getCinema().getHall().getRowsAmount() - getCinema().getHall().getLastFullPriceSeatsRowPosition();
